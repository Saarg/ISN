...
void Spawn::create()
{
    if(m_clock.getElapsedTime() > sf::seconds(1/6))
    {
        ...
        if(m_Random < 10){
            ...
            m_entity_tab->at(m_entity_tab->size()-1)->setPosition(rand()%800, -50);//SpawnX=random, le Y = -50 permet une arrivé plus "fluide"
        }
            ...
            m_entity_tab->at(m_entity_tab->size()-1)->setPosition(rand()%800, -50);
        }
